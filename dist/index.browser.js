...
var HYBRID_TYPE = 0; // eg: !01a3A

var ALPHABETIC_AND_DIGITAL_TYPE = 1; // eg: 001a3A

var ALPHABETIC_TYPE = 2; // eg: eaAeeB

var DIGITAL_TYPE = 3; // eg: 104730
  ...
  (0, _createClass2.default)(PasswordGeneratorCore, [{
    ...
    value: function checkParams(params) {
      function checkPasswordType(params) {
        ...
        if (params.passwordType != HYBRID_TYPE && params.passwordType != ALPHABETIC_AND_DIGITAL_TYPE && params.passwordType != ALPHABETIC_TYPE && params.passwordType != DIGITAL_TYPE) {
    ...
    value: function generate() {
      ...
      if (params.passwordType == DIGITAL_TYPE) {
        ...
        return (0, _slice.default)(_context2 = text.match(/[a-zA-Z]/g)).call(_context2, 0, params.passwordLength).join('');
        ...
        // determine whether the text string contains uppercase letters, lowercase letters and numbers, then cut into pieces according to the required length
        var _result2 = text.match(new RegExp("(?=.*\\d)(?=.*[A-Z])(?=.*[a-z]).{".concat(params.passwordLength, "}"), 'g'));

        try {
          var _iterator2 = _createForOfIteratorHelper(_result2),
              _step2;
          ...
          try {
            for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
              ...
              // find a element which meet the requirements from the result array
              var _result3 = new RegExp("(?=.*\\d)(?=.*[A-Z])(?=.*[a-z])").test(_item);

              if (_result3) {
                // match all numbers
                var numberMatchResult = _item.match(/\d/g); // match all lowercase letters


                var lLetterMatchResult = _item.match(/[a-z]/g); // match all uppercase letters


                var uLetterMatchResult = _item.match(/[A-Z]/g); // match a special symbol according to the first element belongs to the numberMatchResult array
                ...
                if (numberMatchResult.length > lLetterMatchResult.length && numberMatchResult.length > uLetterMatchResult.length) {
                  ...
                  return _item.replace(numberMatchResult[0], replacement);
                  ...
                  return _item.replace(lLetterMatchResult[0], replacement);
                } else {
                  // the most numerous chars are uppercase letters, replace the first uppercase letter with the special symbol
                  return _item.replace(uLetterMatchResult[0], replacement);
                }
              }
            }
          } catch (err) {
