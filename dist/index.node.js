...
class PasswordGeneratorCore {
  ...
  getComplex(params) {
    const raw = this.hmacsha512(params.privateKey, params.publicKey, params.timestamp);
    const origin = Object.freeze(this.getOrigin(raw, params.privateKey, params.timestamp));
    const ruler = Object.freeze(this.getRuler(raw, params.privateKey, params.timestamp));
    const complex = [];
  ...
  generate(text = '', params = {}) {
    if (params.passwordType == DIGITAL_TYPE) {
      ...
      return text.match(/[a-zA-Z]/g).slice(0, params.passwordLength).join('');
      ...
      const result = text.match(new RegExp(`(?=.*\\d)(?=.*[A-Z])(?=.*[a-z]).{${params.passwordLength}}`, 'g'));
      ...
      try {
        for (const item of result) {
          ...
          if (result) {
            // match all numbers
            const numberMatchResult = item.match(/\d/g); // match all lowercase letters

            const lLetterMatchResult = item.match(/[a-z]/g); // match all uppercase letters

            const uLetterMatchResult = item.match(/[A-Z]/g); // match a special symbol according to the first element belongs to the numberMatchResult array
